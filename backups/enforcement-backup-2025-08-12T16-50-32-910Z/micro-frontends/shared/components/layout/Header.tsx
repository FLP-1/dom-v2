  ...
  return (
    <View style={styles.container}>
      <View style={styles.leftSection}>
        {showMenu && (
          <TouchableOpacity style={styles.menuButton} onPress={onMenuPress} accessibilityLabel="Abrir menu">
            <Icon name="menu-outline" size={iconSize + 2} color="#495057" />
          </TouchableOpacity>
        )}
        <View style={styles.titleContainer}>
          ...
          {user && (
            <View style={{ flexDirection: 'row', alignItems: 'center', gap: 8 }}>
              <Text style={[styles.subtitle, { fontSize: subtitleFontSize }]}>{user.name} · {user.profile}</Text>
              <View style={[styles.profileBadge, (user.profile || '').toLowerCase() === 'admin' ? styles.badgeAdmin : styles.badgeDefault]}>
                <Text style={styles.badgeText}>{(user.profile || '').toUpperCase()}</Text>
              </View>
            </View>
          )}
        </View>
      ...
      <View style={styles.rightSection}>
        {showLogout && onLogout && (
          <TouchableOpacity style={styles.logoutButton} onPress={onLogout} accessibilityLabel="Sair">
            <Icon name="log-out-outline" size={iconSize} color="#ffffff" />
          </TouchableOpacity>
        )}
      </View>
    </View>
...
const styles = StyleSheet.create({
  ...
  logoutButton: { width: 40, height: 40, borderRadius: 8, backgroundColor: '#dc3545', justifyContent: 'center', alignItems: 'center' },
});
