...
export default function dynamic<T = any>(loader: Loader<T>) {
  const Lazy = React.lazy(async () => {
    const mod: unknown = await loader();
    if (mod && 'default' in mod) return mod as any;
    return { default: mod } as any;
  });

  const DynamicComponent: React.FC<T> = (props: unknown) => (
    <Suspense fallback={null}>
      <Lazy {...props} />
    </Suspense>
  );

  return DynamicComponent as React.ComponentType<T>;
}
