# Script de Isolamento Simples - DOM v2
...
Write-Host "Criando diretorio de isolamento: $LegacyDir" -ForegroundColor Yellow
...
foreach ($item in $ObsoleteItems) {
    ...
    if ($matchingItems) {
        ...
        Write-Host "Nenhum item encontrado para: $source" -ForegroundColor Yellow
    }
}
...
# Criar arquivo de inventário simples
$InventoryFile = "$LegacyDir/inventory.txt"
$InventoryContent = @"
...
Write-Host "Diretorio: $LegacyDir" -ForegroundColor Cyan
Write-Host "Inventario: $InventoryFile" -ForegroundColor Cyan

Write-Host ""
Write-Host "ISOLAMENTO CONCLUIDO!" -ForegroundColor Green
