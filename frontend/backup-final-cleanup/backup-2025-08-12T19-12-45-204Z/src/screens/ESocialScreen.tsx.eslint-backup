...
const ESocialScreen: React.FC = () => {
  ...
  // Handlers
  const handleCreateCertificate = useCallback(async (certificateData: any) => {
    try {
  ...
  const handleUpdateConfig = useCallback(async (configData: any) => {
    try {
  ...
  if (loading && !certificates.length && !config) {
    return (
