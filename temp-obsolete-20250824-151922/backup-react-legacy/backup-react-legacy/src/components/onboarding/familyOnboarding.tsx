/**
 * @fileoverview Membro da Família Onboarding - Componente de onboarding
 * @description Onboarding personalizado para perfil Membro da Família
 * @version 2.0.0
 * @generated 2025-08-10T02:36:53.691Z
 */
...
export const Membro da FamíliaOnboarding: React.FC<OnboardingProps> = ({
  userId,
  onComplete,
  onSkip
}) => {
  ...
  const loadOnboardingProgress = async () => {
    try {
      const progress = await onboardingService.getProgress(userId, 'family');
  ...
  const saveProgress = async () => {
    try {
      await onboardingService.saveProgress(userId, 'family', {
        currentStep,
        stepData,
        updatedAt: new Date()
      });
  ...
  const completeOnboarding = async () => {
    try {
      await onboardingService.markComplete(userId, 'family');
...
export default Membro da FamíliaOnboarding;
