...
export class IntegrationService {
  ...
  async getAddressByCEP(cep: string): Promise<ViaCEPResponse> {
    try {
      ...
      if (data.erro) { /* TODO: Implement error handling */ } return data;
    } catch (error) {
  ...
  async generateESocialXML(eventType: string, employeeData: unknown): Promise<string> {
    try {
  ...
  // DEPRECATED: utilizar endpoints reais do backend /api/v1/integrations
  getMockESocialEvents(): ESocialEvent[] { return []; }

  getMockStripePayments(): StripePayment[] { return []; }

  getMockSPTransRoutes(): SPTransRoute[] { return []; }

  getMockTimeCardEntries(): TimeCardEntry[] { return []; }
}
