# Compliance Checker Simple - DOM v2
# Verifica conformidade com regras, diretrizes e boas praticas do projeto
...
function Test-Architecture-Compliance {
    ...
    # Verificar se nao ha frameworks proibidos
    ...
    if (Test-Path $packageJson) {
        ...
        foreach ($framework in $prohibitedFrameworks) {
            if ($content -match $framework) {
                Register-Violation "Arquitetura" "Framework proibido detectado: $framework" $packageJson "" "Remover dependencia do $framework"
            } else {
                Register-Success "Framework proibido $framework nao encontrado"
            }
        }
    }
    
    # Verificar tecnologias obrigatorias
    $requiredTech = @("node", "typescript", "prisma", "postgresql")
    foreach ($tech in $requiredTech) {
        if ($content -match $tech) {
            Register-Success "Tecnologia obrigatoria $tech encontrada"
        } else {
            Register-Violation "Arquitetura" "Tecnologia obrigatoria nao encontrada: $tech" $packageJson "" "Adicionar $tech ao projeto"
        }
    }
}
...
function Test-Critical-Thinking-Compliance {
    ...
    # Verificar validacao e tratamento de erros
    ...
    foreach ($file in $jsFiles) {
        ...
        if ($content -match "try\s*\{|catch\s*\(|throw\s+new\s+Error") {
            ...
            Register-Violation "Pensamento Critico" "Tratamento de erros ausente em $($file.Name)" $file.FullName "" "Implementar try-catch e validacoes"
        }
    }
    
    # Verificar validacao de dados
    $htmlFiles = Get-ChildItem -Path "frontend" -Filter "*.html" -Recurse
    foreach ($file in $htmlFiles) {
        $content = Get-Content $file.FullName -Raw
        if ($content -match "required|pattern|minlength|maxlength") {
            Register-Success "Validacao HTML encontrada em $($file.Name)"
        } else {
            Register-Violation "Pensamento Critico" "Validacao HTML ausente em $($file.Name)" $file.FullName "" "Adicionar atributos de validacao"
        }
    }
}
...
function Test-Structure-Compliance {
    ...
    # Verificar organizacao modular
    $frontendDirs = @("frontend/css", "frontend/js", "frontend/components", "frontend/assets")
    foreach ($dir in $frontendDirs) {
        if (Test-Path $dir) {
            Register-Success "Diretorio modular $dir existe"
        } else {
            Register-Violation "Estrutura" "Diretorio modular nao encontrado: $dir" "" "" "Criar diretorio $dir"
        }
    }
    ...
    if ($jsFiles.Count -gt 0) {
        ...
        Register-Violation "Estrutura" "Nenhum arquivo JavaScript separado encontrado" "" "" "Separar logica em arquivos JS"
    }
    ...
    if ($docs.Count -gt 0) {
        Register-Success "Documentacao encontrada"
    } else {
...
function Start-ComplianceCheck {
    ...
    Write-Host "`n" + "=" * 60 -ForegroundColor Gray
    Write-Host "Verificacao de conformidade concluida" -ForegroundColor Green
}
