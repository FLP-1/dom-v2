/**
 * @fileoverview Empregador/Família Onboarding - Componente de onboarding
 * @description Onboarding personalizado para perfil Empregador/Família
 * @version 2.0.0
 * @generated 2025-08-10T02:36:53.684Z
 */
...
export const Empregador/FamíliaOnboarding: React.FC<OnboardingProps> = ({
  userId,
  onComplete,
  onSkip
}) => {
  ...
  const loadOnboardingProgress = async () => {
    try {
      const progress = await onboardingService.getProgress(userId, 'employer');
  ...
  const saveProgress = async () => {
    try {
      await onboardingService.saveProgress(userId, 'employer', {
        currentStep,
        stepData,
        updatedAt: new Date()
      });
  ...
  const completeOnboarding = async () => {
    try {
      await onboardingService.markComplete(userId, 'employer');
...
export default Empregador/FamíliaOnboarding;
