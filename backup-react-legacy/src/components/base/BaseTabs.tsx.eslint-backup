...
export const BaseTabs: React.FC<BaseTabsProps> = memo(({
  ...
  // Handle tab change
  const handleTabChange = useCallback((key: string, event?: any) => {
  ...
  // Handle tab close
  const handleTabClose = useCallback((key: string, event: any) => {
  ...
  // Handle add tab
  const handleAddTab = useCallback((event: any) => {
    if (onEdit) {
      onEdit(event, 'add');
    }
  }, [onEdit]);
  ...
  const tabBarComponent = renderTabBar ? 
    renderTabBar({}, renderTabBarContent) : 
    renderTabBarContent();
