...
export const useESocialData = () => {
  ...
  const loadCertificates = async () => {
    try {
      ...
      const convertedCertificates: ESocialCertificate[] = response.data?.map((cert: unknown) => ({
        id: cert.id,
  ...
  const loadEvents = async () => {
    try {
      ...
      const convertedEvents: ESocialEvent[] = response.data?.map((event: unknown) => ({
        id: event.id,
        eventType: event.event_type,
        eventStatus: event.event_status || 'pending',
        eventData: event.event_data || { /* TODO: Implement error handling */ } ,
  ...
  const createCertificate = async (certificateData: unknown) => {
    try {
