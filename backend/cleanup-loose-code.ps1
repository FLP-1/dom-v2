...
Write-Host "🧹 Iniciando limpeza de código solto..." -ForegroundColor Yellow
...
foreach ($file in $files) {
    ...
    foreach ($line in $lines) {
        ...
        if ($trimmedLine -match "^if \(!validateType\(data" -or
            ...
            $trimmedLine -match "^assert\(" -or
            $trimmedLine -match "^try \{" -and $trimmedLine -notmatch "function" -or
            $trimmedLine -match "^catch \(" -and $trimmedLine -notmatch "function") {
            continue
        }
