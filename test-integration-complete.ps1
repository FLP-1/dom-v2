# Teste de Integração Completa - DOM v2
# Script para validar funcionamento do sistema

Write-Host "🧪 TESTE DE INTEGRAÇÃO COMPLETA - DOM v2" -ForegroundColor Cyan
...
function Test-API-Data {
    ...
    try {
        ...
        if ($data.success -eq $true) {
            ...
            Write-Host "   Dados recebidos: $($data.data | ConvertTo-Json -Depth 1)" -ForegroundColor Gray
            return $true
        } else {
...
Write-Host "🚀 INICIANDO TESTES DE INTEGRAÇÃO..." -ForegroundColor Magenta
Write-Host ""
...
try {
    ...
    if ($data.success -eq $true) {
        ...
        Write-Host "❌ FALHA: Login API - Credenciais inválidas" -ForegroundColor Red
    }
}
catch {
...
if ($successRate -eq 100) {
    ...
    Write-Host "🚀 Sistema DOM v2 está funcionando perfeitamente!" -ForegroundColor Green
    ...
    Write-Host "🚨 Verificação urgente necessária" -ForegroundColor Red
}
...
Write-Host "   Email: admin@dom-v2.com" -ForegroundColor White
